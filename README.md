...
-- 5. Reports for machines physically in use but not tracked by the bot
...
4. **Environment Variables**: Add the 5 variables from your `.env` file to Render
5. **Set Webhook**: Once deployed, register both the URL and `TELEGRAM_WEBHOOK_SECRET`:
   ```
   curl -F "url=https://<YOUR_RENDER_URL>/webhook" \\
